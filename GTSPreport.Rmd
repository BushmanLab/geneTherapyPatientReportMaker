...
barplotAbunds <- left_join(
    barplotAbunds, n_timepoints_for_cell_type, by='CellType') %>%
  mutate(maskedRefGeneName = factor(
    maskedRefGeneName, levels = rev(names(siteColors))))
...
barplotAbundsBySample <- left_join(
  barplotAbundsBySample, n_timepoints_for_cell_type, by='CellType') %>%
  mutate(maskedRefGeneName = factor(
    maskedRefGeneName, levels = rev(names(siteColors))))
